...
from apps.participation.models import Team, Invitation
from apps.participation.serializers import (
    TeamListSerializer, TeamSerializer, InvitationListSerializer,
    InvitationSerializer, InvitationActionSerializer
)
from apps.participation.permissions import (
    UserHasParticipant, UserHasTeamInContest, SignupOpen
)
...
class TeamAPIView(GenericAPIView):
    ...
    permission_classes = [
        ...
        UserHasTeamInContest, SignupOpen
    ]

    def get(self, request, contest_id):
        contest = get_object_or_404(Contest, id=contest_id)
        if contest.team_size == 1:
            return Response(data={'detail': 'requested contest is individual'},
                            status=406)
...
class InvitationAPIView(GenericAPIView):
    ...
    def get(self, request, contest_id):
        ...
        data = self.get_serializer(
            request.user.participant.teams.get(contest=contest)).data
        return Response(data)
    ...
    def put(self, request, contest_id):
        ...
        serializer = self.get_serializer(data=request.data)
        if serializer.is_valid(raise_exception=True):
            invitation = get_object_or_404(self.get_queryset(),
