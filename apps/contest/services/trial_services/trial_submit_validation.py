...
class TrialSubmitValidation:
    ...
    def _validate_single_answer(self, submission):
        ...
        if answer_type == AnswerDataTypes.INTEGER:
            try:
                ...
                self._errors[submission.question_id] = trial_submit_exception.ErrorMessages.ANSWER_TYPE_ERROR.format(
                    answer_type)
                self._valid = False
    ...
    def _validate_multi_answer(self, submission):
        ...
        if len(answers) > answer_count:
            self._errors[
                submission.question_id] = trial_submit_exception.ErrorMessages.MULTI_ANSWER_LIMIT_REACHED_ERROR.format(
                answer_count)
            self._valid = False
            return
        for answer in answers:
            if answer_type == AnswerDataTypes.INTEGER:
                try:
                    ...
                    self._errors[
                        submission.question_id] = trial_submit_exception.ErrorMessages.ANSWER_TYPE_ERROR.format(
                        answer_type)
                    self._valid = False
                    break
            elif answer_type == AnswerDataTypes.DOUBLE:
    ...
    def _validate_single_select(self, submission):
        try:
            ...
            return
        if len(label) >= 2:
            self._errors[submission.question_id] = trial_submit_exception.ErrorMessages.SINGLE_SELECT_ERROR
            self._valid = False
            return
    ...
    def _validate_multi_select(self, submission):
        ...
        if len(labels) > answer_count:
            self._errors[
                submission.question_id] = trial_submit_exception.ErrorMessages.MULTI_SELECT_LIMIT_REACHED_ERROR.format(
                answer_count)
            self._valid = False
            return
    ...
    def _validate_file_upload(self, submission):
        ...
        try:
            ...
            self._valid = False
            self._errors[submission.question_id] = f'file with name {submission.id} does not exists'
            return
        ...
        if file_format != answer_file_format:
            self._errors[submission.question_id] = trial_submit_exception.ErrorMessages.INVALID_FILE_FORMAT.format(
                file_format)
            self._valid = False
            return
        if answer_file.size > file_size_limit * 1024 * 1024:
            self._errors[submission.question_id] = trial_submit_exception.ErrorMessages.FILE_SIZE_LIMIT_EXCEEDED.format(
                file_size_limit)
            self._valid = False
            return
