...
class Command(BaseCommand):
    ...
    def _handle_add_task_to_milestone(self, options):
        if not options.get('ids') or len(options.get('ids')) != 2:
            print("Please Enter Command Like This: "
                  "--add_task_to_milestone <task_id> <milestone_id>")
        else:
            task_id, milestone_id = \
                options.get('ids')[0], options.get('ids')[1]
            try:
                ...
                print(e)
            try:
                milestone = contest_models.Milestone.objects.get(
                    id=milestone_id)
